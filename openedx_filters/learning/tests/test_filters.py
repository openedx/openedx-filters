...
class TestSupportEnrollmentDataRequestedFilter(TestCase):
    ...
    def test_run_filter_returns_enrollments_unchanged_when_no_pipeline(self):
        ...
        """
        enrollments = [{"course_id": "some-id"}]
        user = Mock()

        result = SupportEnrollmentDataRequested.run_filter(enrollments=enrollments, user=user)

        assert result == (enrollments, user)

    @patch(
        "openedx_filters.tooling.OpenEdxPublicFilter.run_pipeline",
        return_value={
            "enrollments": [{"course_id": "some-id", "enterprise_course_enrollments": []}],
            "user": Mock(),
        },
    )
    def test_run_filter_returns_enrollments_from_pipeline(self, mock_run_pipeline):
        ...
        """
        result = SupportEnrollmentDataRequested.run_filter(enrollments=[{"course_id": "some-id"}], user=Mock())

        assert result == (
            mock_run_pipeline.return_value["enrollments"],
