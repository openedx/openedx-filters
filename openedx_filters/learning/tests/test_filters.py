...
class TestInstructorDashboardTabsGenerated(TestCase):
    ...
    def test_run_filter_with_pipeline_returning_dict_without_tabs_key(self):
        """
        Test InstructorDashboardTabsGenerated filter when pipeline returns dict without tabs key.

        Expected behavior:
            - The filter should return the original tabs as fallback.
        """
        tabs = [
            {"tab_id": "courseware", "title": "Course", "url": "/course/123", "sort_order": 0},
        ]
        ...
        with patch("openedx_filters.tooling.OpenEdxPublicFilter.run_pipeline") as mock_run_pipeline:
            mock_run_pipeline.return_value = {"course": course, "user": user, "course_key": course_key}
            result_tabs = InstructorDashboardTabsGenerated.run_filter(
                tabs=tabs, course=course, user=user, course_key=course_key
            )
    ...
    def test_run_filter_with_pipeline_returning_dict_with_non_list_tabs(self):
        """
        Test InstructorDashboardTabsGenerated filter when pipeline returns dict with non-list tabs.
    ...
    def test_run_filter_with_pipeline_returning_non_dict(self):
        """
        Test InstructorDashboardTabsGenerated filter when pipeline returns non-dict.
        ...
        with patch("openedx_filters.tooling.OpenEdxPublicFilter.run_pipeline") as mock_run_pipeline:
            mock_run_pipeline.return_value = "not_a_dict"
            result_tabs = InstructorDashboardTabsGenerated.run_filter(
                tabs=tabs, course=course, user=user, course_key=course_key
            )
